...
**🟢 LIVE dal 2026-07-21.** SQL applicati su produzione (verifica: `chat_fns=4`, `ordine_messaggi` ok, `ordini-media` `public=true`, 2 policy) + deploy su `main` (commit `14001b1`, GitHub Pages).

**FIX 2026-07-21 (upload immagini chat/detector davano 403).** Causa: l'header `x-upsert:true` nelle POST di upload → lo storage richiede anche il permesso di UPDATE (che anon non ha) → 403 "violates RLS" fuorviante. Rimosso `x-upsert` da traccia `chatUpload`, Admin `acUpload`, Configuratore `uploadDetectorMedia` (path già unici → upsert inutile). Policy `ordini-media` insert messa `to public` + regex path (la chiave `sb_publishable_` non mappa affidabilmente ad anon/authenticated per lo storage). Verificato live: upload 200, lettura pubblica 200, path non-uuid bloccato 400. NB: `uploadStlToStorage`/`uploadZipGetLink` non erano affetti (già senza x-upsert).

**[DA FARE] Rimasto:** test dal vivo su device (HEIC→JPG da iPhone, anteprima STL); far avanzare le fasi per-item / avvisare il cliente all'identificazione (per ora l'operatore usa i pulsanti di stato esistenti).
